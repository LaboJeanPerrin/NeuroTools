...
try
    ...
    addParameter(p, 'LengthUnit', this.Units.Length, @(x) ismember(x, {'m', 'mm', 'um', [char(181) 'm']}));
...
for i = 1:numel(File)
    ...
    switch mode
        ...
        case 'Signals'
            ...
            % Default values
            res = regexp(line, 'Default\s+DS(\d+)\s+([\d\.]+)', 'tokens');
            if ~isempty(res)
            ...
            % Digital signals
            res = regexp(line, 'DS(\d+)\s+([\d\.]+)\s+([\d\.]+)', 'tokens');
            if ~isempty(res)
                j = str2double(res{1}{1});
...
switch unit
    ...
    case 'm'
        switch target
            case 'm', out = val;
            case 'mm', out = val*1e3;
            case {'um', '�m', 'µm', [char(181) 'm']}, out = val*1e6;
        end
    case 'mm'
        switch target
            case 'm', out = val*1e-3;
            case 'mm', out = val;
            case {'um', '�m', 'µm', [char(181) 'm']}, out = val*1e3;
        end
    case {'um', '�m', 'µm', [char(181) 'm']}
        switch target
            case 'm', out = val*1e-6;
            case 'mm', out = val*1e-3;
            case {'um', '�m', 'µm', [char(181) 'm']}, out = val;
        end
    ...
    case 'm/V'
        switch target
            ...
            case {'um/V', '�m/V', 'µm/V', [char(181) 'm/V']}, out = val*1e6;
        end
    case 'mm/V'
        switch target
            case 'm/V', out = val*1e-3;
            case 'mm/V', out = val;
            case {'um/V', '�m/V', 'µm/V', [char(181) 'm/V']}, out = val*1e3;
        end
    case {'um/V', '�m/V', 'µm/V', [char(181) 'm/V']}
        switch target
            case 'm/V', out = val*1e-6;
            case 'mm/V', out = val*1e-3;
            case {'um/V', '�m/V', 'µm/V', [char(181) 'm/V']}, out = val;
        end
